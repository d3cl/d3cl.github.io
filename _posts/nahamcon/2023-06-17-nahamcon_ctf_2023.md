...
tags: [reflected-xss, local-file-inclusion, server-side-request-forgery, css-injection, os-command-injection, server-side-template-injection, privilege-escalation, date, private-key]
...
We can perform an LFI through the `artifact` parameter. `browse?artifact=/./etc/passwd` will provide us the `etc/passwd` file.

![lfi](/assets/img/nahamctf-museum-lfi.png)

We can find the location of the source code via `browse?artifact=/./proc/self/cmdline`.
